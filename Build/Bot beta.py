...
# Spending advice is low-frequency (only fires when asked) and quality
# matters more than speed/cost here — worth the stronger, pricier tier.
ADVICE_MODEL_NAME = "gemini-3.5-flash"
...
COLUMN_MIGRATIONS = [
    ...
    ("profiles", "import_cooldown_days", "INTEGER NOT NULL DEFAULT 90"),
...
def set_budget(profile_id: int, category: str, amount: float):
    conn = get_db()
    conn.execute(
        """INSERT INTO budgets (profile_id, category, amount, updated_at)
           VALUES (?, ?, ?, ?)
           ON CONFLICT(profile_id, category) DO UPDATE SET
             amount = excluded.amount, updated_at = excluded.updated_at""",
        (profile_id, category, amount, datetime.now().isoformat()),
    )
    conn.commit()
    conn.close()


def get_budget(profile_id: int, category: str) -> dict:
    conn = get_db()
    row = conn.execute(
        "SELECT amount, period_days FROM budgets WHERE profile_id = ? AND category = ?",
        (profile_id, category),
    ).fetchone()
    conn.close()
...
def get_all_budgets(profile_id: int) -> list:
    conn = get_db()
    rows = conn.execute(
        "SELECT category, amount, period_days FROM budgets WHERE profile_id = ?",
        (profile_id,),
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_budget_status(profile_id: int, category: str) -> dict:
    """Compares actual spend (using the same grouping-aware logic as
    normal category queries) against a set budget goal."""
    budget = get_budget(profile_id, category)
    if not budget:
        return None
    if category == "overall":
        conn = get_db()
        cutoff = (datetime.now() - timedelta(days=budget["period_days"])).isoformat()
        row = conn.execute(
            """SELECT COALESCE(SUM(ri.amount),0) as total FROM receipt_items ri
        ...
        spent = get_category_spend(profile_id, category, budget["period_days"])["total"]

    remaining = budget["amount"] - spent
    return {
        "category": category, "budget": budget["amount"], "spent": spent,
        "remaining": remaining, "over": remaining < 0, "period_days": budget["period_days"],
    }
...
def get_category_spend(profile_id: int, category: str, days: int = 30) -> dict:
    """Sums spend for a category over the trailing N days. If `category`
    is a broad group term (e.g. 'transport'), sums across every
    sub-category in that group instead of matching just one literal
    category name — this is the fix for broad terms silently only
    catching one narrow sub-category."""
    conn = get_db()
    cutoff = (datetime.now() - timedelta(days=days)).isoformat()

    categories_to_match = CATEGORY_GROUPS.get(category, [category])
    placeholders = ",".join("?" * len(categories_to_match))
    row = conn.execute(
        f"""SELECT COALESCE(SUM(ri.amount), 0) as total, COUNT(*) as cnt
           FROM receipt_items ri
           JOIN receipts r ON ri.receipt_id = r.receipt_id
           WHERE r.profile_id = ? AND ri.category IN ({placeholders}) AND r.purchased_at >= ?""",
        (profile_id, *categories_to_match, cutoff),
    ).fetchone()
    conn.close()
    return {"total": row["total"], "count": row["cnt"]}
...
- "set_budget": user wants to set or change a spending goal/budget (e.g. "set my grocery budget to 2000", "budget 15000 overall this month")
- "budget_status": user wants to know how they're doing against a budget goal (e.g. "how am I doing on my budget", "am I over on groceries")
- "spending_advice": user is asking where they can save money or how to budget better (e.g. "where can I save money", "how can I spend less")
...
Reply with ONLY one word: log_transaction, log_income, category_query, income_query, balance_query, set_budget, budget_status, spending_advice, delete_last, edit_last, request_file, request_report, or other."""

PARSE_SET_BUDGET_PROMPT = """The user wants to set a spending budget/goal.
Reply in EXACTLY this format, nothing else:
category: <one of: groceries, client_entertainment, staff_meals, fuel, vehicle_maintenance, parking_tolls, travel_flights, health, entertainment, electricity, water, telecoms_mobile, insurance, professional_services, repairs_maintenance, household, clothes, transport, eating_out, food, utilities, medical, or "overall" if they didn't name a specific category>
amount: <numeric amount only, no currency symbol>

Message: "{message}"
"""

PARSE_BUDGET_STATUS_PROMPT = """The user is asking about their budget status.
Reply in EXACTLY this format, nothing else:
category: <one of: groceries, client_entertainment, staff_meals, fuel, vehicle_maintenance, parking_tolls, travel_flights, health, entertainment, electricity, water, telecoms_mobile, insurance, professional_services, repairs_maintenance, household, clothes, transport, eating_out, food, utilities, medical, or "overall" if they didn't name a specific category, or "all" if they want every budget's status>
...
amount: <numeric amount only, no currency symbol>
category: <one of: groceries, client_entertainment, staff_meals, fuel, vehicle_maintenance, parking_tolls, travel_flights, health, entertainment, electricity, water, telecoms_mobile, insurance, professional_services, repairs_maintenance, household, clothes, other>
...
specific category, a broad group of categories, or an overall total if they didn't mention one.
Reply in EXACTLY this format, nothing else:
category: <one of: groceries, client_entertainment, staff_meals, fuel, vehicle_maintenance, parking_tolls, travel_flights, health, entertainment, electricity, water, telecoms_mobile, insurance, professional_services, repairs_maintenance, household, clothes, other, transport, eating_out, food, utilities, medical, or "all" if not specific>
...
Use the BROAD term (transport, eating_out, food, utilities, medical) whenever the user's
wording is broad rather than naming one specific thing — e.g. "how much on transport" should
be "transport", not a guess at one specific sub-category like "fuel".
...
CATEGORY_KEYWORDS = {
    ...
    "household": ["takealot", "gadget", "gadgettime", "evetech", "microsoft",
                   "godaddy", "openai"],
    "clothes": ["mr price", "edgars", "truworths", "woolworths clothing", "cotton on",
                "identity", "markham", "jet ", "ackermans", "sportscene", "totalsports"],
}

# Broad terms map to a GROUP of sub-categories, so "how much on transport"
# correctly sums fuel + vehicle_maintenance + parking_tolls + travel_flights
# instead of Gemini guessing just one of them and silently missing the rest.
CATEGORY_GROUPS = {
    "transport": ["fuel", "vehicle_maintenance", "parking_tolls", "travel_flights"],
    "eating_out": ["client_entertainment", "staff_meals"],
    "food": ["groceries", "client_entertainment", "staff_meals"],
    "utilities": ["electricity", "water", "telecoms_mobile"],
...
async def classify_intent(message_text: str) -> str:
    ...
    valid = ("log_transaction", "log_income", "category_query", "income_query",
              "balance_query", "set_budget", "budget_status", "spending_advice",
              "delete_last", "edit_last", "request_file", "request_report", "other")
    if intent not in valid:
        return "other"
    return intent


async def parse_set_budget(message_text: str) -> dict:
    response = gemini_client.models.generate_content(
        model=GEMINI_MODEL_NAME,
        contents=PARSE_SET_BUDGET_PROMPT.format(message=message_text),
    )
    lines = response.text.strip().split("\n")
    parsed = {}
    for line in lines:
...
async def parse_budget_status_query(message_text: str) -> dict:
    response = gemini_client.models.generate_content(
        model=GEMINI_MODEL_NAME,
        contents=PARSE_BUDGET_STATUS_PROMPT.format(message=message_text),
    )
    lines = response.text.strip().split("\n")
    parsed = {}
    for line in lines:
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if intent == "log_transaction":
        ...
        parsed = await parse_set_budget(message_text)
        category = parsed.get("category", "overall")
        try:
            amount = float(parsed.get("amount", "0"))
        except ValueError:
            amount = 0
        ...
        if amount <= 0:
            await update.message.reply_text(
                "Couldn't catch an amount there — try again, e.g. "
                "\"set my grocery budget to 2000\"."
            )
            return

        set_budget(profile_id, category, amount)
        label = "overall spending" if category == "overall" else category
        await update.message.reply_text(
            f"🎯 Budget set: R{amount:.2f}/month for {label}."
        )

    elif intent == "budget_status":
        parsed = await parse_budget_status_query(message_text)
        category = parsed.get("category", "overall")

        if category == "all":
            budgets = get_all_budgets(profile_id)
            if not budgets:
                await update.message.reply_text(
                    "No budgets set yet — try \"set my grocery budget to 2000\" to get started."
                )
                return
            lines = ["🎯 Your budgets:"]
            for b in budgets:
                status = get_budget_status(profile_id, b["category"])
                emoji = "🔴" if status["over"] else "🟢"
                lines.append(
                    f"{emoji} {b['category']}: R{status['spent']:.2f} / R{status['budget']:.2f}"
                )
            await update.message.reply_text("\n".join(lines))
        else:
            status = get_budget_status(profile_id, category)
            if not status:
                await update.message.reply_text(
                    f"No budget set for {category} yet — try \"set my {category} budget to 2000\"."
                )
            else:
                if status["over"]:
                    await update.message.reply_text(
                        f"🔴 {category}: R{status['spent']:.2f} / R{status['budget']:.2f} — "
                        f"R{abs(status['remaining']):.2f} OVER budget this month."
                    )
                else:
                    await update.message.reply_text(
                        f"🟢 {category}: R{status['spent']:.2f} / R{status['budget']:.2f} — "
                        f"R{status['remaining']:.2f} left this month."
                    )

    elif intent == "spending_advice":
        recent_summary = get_recent_summary(profile_id, limit=30)
        budgets = get_all_budgets(profile_id)
        recurring = conn = get_db()
        recurring_rows = conn.execute(
            "SELECT merchant, typical_amount FROM recurring_transactions WHERE profile_id = ? AND active = 1",
            (profile_id,),
        ).fetchall()
        conn.close()

        context_lines = ["Recent transactions:"]
        for t in recent_summary[:20]:
            context_lines.append(f"  R{t['total_amount']:.2f} — {t['merchant']} ({t['purchased_at'][:10]})")
        if budgets:
            context_lines.append("\nCurrent budgets:")
            for b in budgets:
                context_lines.append(f"  {b['category']}: R{b['amount']:.2f}/month")
        if recurring_rows:
            context_lines.append("\nRecurring payments:")
            for r in recurring_rows:
                context_lines.append(f"  R{r['typical_amount']:.2f}/month — {r['merchant']}")

        advice_prompt = (
            "You are a personal finance assistant. Based on this person's real spending data, "
            "give specific, genuinely useful advice on where they could save money. Be concrete — "
            "reference actual numbers and merchants from their data, not generic tips. Keep it to "
            "2-3 short paragraphs.\n\n" + "\n".join(context_lines)
        )
        response = gemini_client.models.generate_content(
            model=ADVICE_MODEL_NAME, contents=advice_prompt,
        )
